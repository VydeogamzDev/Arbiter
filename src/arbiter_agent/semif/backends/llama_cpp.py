...
- Scoring: one forward pass with ``n_predict: 1`` and top-k token probabilities; the score of
...
class LlamaCppBackend:
    ...
    def __init__(self, base_url: str = "http://127.0.0.1:8088", revision: str | None = None,
                 adapters: dict[str, int] | None = None, timeout_s: float = 30.0) -> None:
        ...
        self.max_tokens = int(settings.get("n_ctx") or props.get("n_ctx") or 4096)
    ...
    def _letter_probs(resp: dict[str, Any]) -> dict[str, float]:
        ...
        for item in first.get("top_logprobs") or []:                # newer format (log-probs)
    ...
    def score(self, requests: list[ScoreRequest], deadline: Deadline) -> list[ScoreResult]:
        ...
        for r in requests:
            ...
            if deadline.expired():
                ...
                continue
            body: dict[str, Any] = {"prompt": r.prompt, "n_predict": 1, "n_probs": TOP_K, "temperature": 0.0,
