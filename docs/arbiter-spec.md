...
semif:
  ...
  encoder:                      # tier 0: short typed decisions on CPU or GPU, no GPU required (decision 0026)
    enabled: false
    model: fastino/GLiNER2.5-Decide   # a Hub id or a local folder (weights, ONNX export or Core ML packages)
    runtime: auto               # auto | torch | onnx | coreml; auto picks from what the folder holds
    precision: w8               # coreml package precision (fp16 | w8 | lut6); onnx takes its export's
    device: auto                # auto | cpu | cuda (torch runtime)
